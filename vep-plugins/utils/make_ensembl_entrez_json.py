...
def load_gencode(fil):
    ...
    # Open
    if fil.endswith('gz'): fh = gzip.open(fil, 'rb')
    else: fh = gzip.open(fil, 'rU')
...
def load_ncbi(fil):
    ...
    for line in fh: 
        ...
        if line.startswith('#'): continue
            ...
            entrez = int(cols[1])
            symbol = cols[2]
            if symbol not in dic: dic[symbol] = []
...
<ftp://ftp.sanger.ac.uk/pub/gencode/Gencode_human/release_22/gencode.v22.metadata.EntrezGene.gz>
